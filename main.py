...
def update_star(star: schemas.Star, db: Session = Depends(get_db)):
    ...
    if db_star is None:
        raise HTTPException(status_code=404, detail="Star non trouvée")
    return db_star
...
#ajout acteur a 1 film
@app.post("/movies/actor/", response_model=schemas.MovieDetail)
def add_an_actor(mid: int, sid: int, db: Session = Depends(get_db)):
    db_actor = crud.add_an_actor(db=db, movie_id=mid, star_id=sid)
    return db_actor

#remplace les acteurs d'un film
@app.put("/movies/actors/", response_model=schemas.MovieDetail)
def update_actors(mid: int, sids: List[int], db: Session = Depends(get_db)):
    db_movie = crud.update_actors(db=db, movie_id=mid, stars_id=sids)
    return db_movie


#Question 3

#année du premier et dernier film de chaque acteurs + nombre de films avec seuil mini de film
@app.get("/stars/stats_first_last_movie_by_actor")
def read_stats_first_last_movie_by_actor(minc: Optional[int] = 7, db: Session = Depends(get_db)):
    return crud.get_stats_first_last_movie_by_actor(db=db, min_count=minc)
