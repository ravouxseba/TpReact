...
def add_an_actor(db: Session, movie_id: int, star_id: int):
    db_movie = get_movie(db=db, movie_id=movie_id)
    db_star = get_star(db=db, star_id=star_id)
    if db_movie is None or db_star is None:
        return None
...
def update_actors(db: Session, movie_id: int, stars_id: List[int]):
    db_movie = get_movie(db=db, movie_id=movie_id)
    if db_movie is None:
        return None
    db_movie.actors = []
...
def get_stats_first_last_movie_by_actor(db: Session, min_count: int):
    return db.query(models.Star,func.min(models.Movie.year),func.max(models.Movie.year),func.count(models.Movie.id).label("movie_count"))\
